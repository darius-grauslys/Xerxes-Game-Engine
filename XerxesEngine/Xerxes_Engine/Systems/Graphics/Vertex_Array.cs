...
{
    public class Vertex_Array : IDisposable
    {
        public static readonly int VERTEXARRAY_INDEX_COUNT = 4;

        public Vertex[] Vertices { get; internal set; }
        private Texture_R2 texture;

        public int VertexBufferObject { get; private set; }

        private bool flippedX, flippedY;

        public Texture_R2 Texture => texture;

        internal Vertex_Array(Vertex[] vertices)
        {
            VertexBufferObject = GL.GenBuffer();
            flippedX = false;
            flippedY = false;
            texture = default(Texture_R2);
            this.Vertices = vertices;
        }

        public Vertex_Array(Texture_R2 texture, Vertex[] vertices = null,
            float r = 0,
            float g = 0,
            float b = 0,
            float a = 0)
